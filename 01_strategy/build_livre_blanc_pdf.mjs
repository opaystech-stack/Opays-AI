...
const markdownPath = path.join(root, 'livre_blanc_opays_tech.md');
const pdfPath = path.join(root, 'export', 'livre_blanc_opays_tech.pdf');
...
function markdownToLines(sourceLines) {
  const buffer = [];
  buffer.push('OPAYS TECH');
  buffer.push('Livre blanc officiel');
  buffer.push('Opays Tech construit trois choses : une methode, un outil, une capacite.');
  buffer.push('');
  buffer.push('Version : Mai 2026');
  buffer.push('Statut : Document de Direction');
  buffer.push('Acces : Associes & Partenaires agrees');
  buffer.push('');
...
function buildContentStream(lines, pageWidth = 612, pageHeight = 792) {
  const marginLeft = 50;
  const marginTop = 742;
  const lineHeight = 14;
  const text = [];
...
async function buildPdf() {
  ...
  await fs.writeFile(pdfPath, Buffer.concat(parts));
  return pdfPath;
}
